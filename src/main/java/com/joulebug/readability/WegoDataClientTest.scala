...
object WegoDataClientTest {
  ...
  def WDCTest() {
    ...
    try {
      val viewLogin = WDC.getViewLogin(151264)
    ...
    try {
    val arrayTwo: Array[WegoDataRawData] = WDC.getMeterRawData(151264)
    for (parsed <- arrayTwo) {
    ...
    try {
      val wegoRawDataPoint = WDC.getMeterRawDataPoint(370, 151264)
  ...
  try {
    val wegoRawDatum = WDC.getMeterRawDatum(151264)
